...
const OutForDeliveryOrdersPage = ({ allOrders, updateOrderStatus }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [dateFilter, setDateFilter] = useState('');

  const filteredOrders = useMemo(() => {
    return allOrders
      .filter(order => order.status === OrderStatus.OUT_FOR_DELIVERY)
      .filter(order => {
  ...
  return (
    <div className="container mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-gray-800">Out for Delivery</h2>
        <span className="text-lg font-semibold bg-yellow-100 text-yellow-800 rounded-full px-4 py-1">
      ...
      {filteredOrders.length > 0 ? (
        ...
        <div className="text-center text-gray-500 p-10 bg-white rounded-lg shadow">
          <p className="text-lg">No orders out for delivery match your criteria.</p>
        </div>
      )}
    </div>
  );
