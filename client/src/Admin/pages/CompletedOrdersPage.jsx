...
const CompletedOrdersPage = ({ allOrders, updateOrderStatus }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [dateFilter, setDateFilter] = useState('');

  const filteredOrders = useMemo(() => {
    return allOrders
      .filter(order => order.status === OrderStatus.DELIVERED || order.status === OrderStatus.REJECTED)
      .filter(order => {
  ...
  return (
    <div className="container mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <span className="text-lg font-semibold bg-gray-200 text-gray-800 rounded-full px-4 py-1">
      ...
      {filteredOrders.length > 0 ? (
        ...
        <div className="text-center text-gray-500 p-10 bg-white rounded-lg shadow">
          <p className="text-lg">No completed orders match your criteria.</p>
        </div>
      )}
    </div>
  );
